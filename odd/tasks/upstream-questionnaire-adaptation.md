...
- [ ] **UQA-04a — Preserve partial cancellation results.** User approved two bounded TDD work units with no size exception: verified RED `a5d49950d7ba2e4a7f2f7faf7227ece90f2dc906`, CI 35604269961, is 379 lines; GREEN is a separate incremental work unit based on that exact commit under the general 400-line cap. RED proved the view supplied committed answers while the producer discarded them; draft omission and UQA-03 behavior passed. Notes remain UQA-06.
- [ ] **UQA-04b — Settle abort and races exactly once.** Handle pre-mount and mounted abort, listener cleanup and abort/submit/cancel races through the real `done` path, with partial answers and balanced events.
...
UQA-04a is next. Build a separate immutable partial-cancellation artifact using cumulative replacement patches against the pinned source, while preserving the UQA-03 manifest and patch files byte-for-byte. Forecast the full RED+GREEN unit under 400 lines, then prove committed-answer retention and draft omission without expanding into abort, notes, RPC or dock controls.
