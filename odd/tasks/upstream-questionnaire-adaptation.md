...
- [ ] **UQA-01 — Acquire and identify current upstream immutably.** Implementation complete; hosted evidence pending. One delegated writer handled three nontrivial files after reading the existing acquisition pattern. Actual scope is 380 additions across `.github/workflows/fixtures/hosted-upstream-questionnaire.json`, `.github/workflows/questionnaire-upstream-adaptation.yml` and `tests/hosted-upstream-questionnaire-acquisition.test.ts`, under the 400-line cap. The separately named lane pins the exact commit/tree, 729-file inventory, lock/package identities and clean source without changing the old pin. No questionnaire behavior or runtime execution.
- [ ] **UQA-02 — Build a strict derived adaptation artifact.** Apply only manifest-listed patches to a disposable copy with exact preimages, no fuzz, no unexpected paths and no shared-module edits. Keep the legacy composer unchanged. Runtime harness N/A: artifact construction only.
...
UQA-01 source is ready for publication and exact-SHA hosted verification. Acceptance requires two acquisition tests, scoped strict TypeScript, actual network-none/read-only isolation and final candidate cleanliness. No upstream questionnaire behavior changes until this immutable source identity passes independently.
