...
- [ ] **UQA-03 — Restore Herdr blocked-event compatibility.** GREEN active after verified hosted RED at `8a701b8a4b9bb0123984c253c8b109365722213c`, CI 35555277959: 92 tests, 91 passed and only the actual producer/Herdr integration failed because compatibility events were absent; all acquisition, composition, type, read-only and cleanliness gates passed. Forecast remains under 400 total lines. Preserve UQA-02's checked-in empty manifest and the separate versioned Herdr adaptation manifest; runtime-synthesized patch authority is forbidden. Emit native and compatibility `rpiv:ask-user:blocked` events from the actual producer without consumer/shared-helper changes.
...
UQA-03 is next. Add the smallest manifest-listed patch and actual upstream producer/Herdr-consumer regression for balanced compatibility events. Forecast the complete behavior-plus-test unit under the general 400-line cap, observe hosted RED on the derived artifact, then implement GREEN without modifying the Herdr consumer or shared interaction helper.
