...
- [ ] **UQA-04b — Settle abort and races exactly once.** User approved readability-safe RED work units A1 → A2 → B with no size exception after rejecting a code-golfed combined candidate. A1 `42939637c2c0a0312e139249e72e500961c1e8f2` is verified authored-only at 391 lines. A2 `1a7deb68fd8e631506b749485d88f139b0e76eda` is verified authored-only at 246 incremental lines with all eight readable abort/race contracts and unchanged prior gates. Exact standard-Git application/postimage and behavioral RED remain pending B. Initial B `44be140c0dc49e332892a1679a002a6809b1eca8` failed infrastructure because Docker stdin omitted `-i`; no abort composition or behavior ran. The stdin correction `ed88d2f18d848eab5544fb6e61d9077bd44ca5dd` proved exact composition/postimages and read-only isolation, then stopped at strict test-harness typing: nullable listeners were forwarded to non-null `.call` signatures, so no abort behavior ran. The next bounded correction is instrumentation typing plus corresponding immutable postimage proofs only. Production `007`/view `008` remain forbidden until B closes.
