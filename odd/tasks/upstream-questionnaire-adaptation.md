...
- [ ] **UQA-06 — Add question and global notes.** Preserve trimmed nonblank `notes` on committed answers and independent `globalNote` in successful and cancelled structured results; request schema remains unchanged and new textual/RPC formatting stays in UQA-09. Split under the 400-line cap: UQA-06a view/result RED→GREEN, then UQA-06b producer passthrough RED→GREEN. UQA-06a RED patch `018` is authored from the immutable UQA-05 final test postimage; hosted observation is pending.
...
UQA-06a is active. Observe the hosted four-test notes-view RED before any view production patch, preserving UQA-05 controls/MULTI behavior, request schema and shared fullscreen interaction.
