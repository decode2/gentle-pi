...
- [ ] **UQA-04b — Settle abort and races exactly once.** User approved readability-safe RED work units A1 → A2 → B with no size exception after rejecting a code-golfed combined candidate. A1 `42939637c2c0a0312e139249e72e500961c1e8f2` is verified authored-only at 391 lines: pre-abort and mounted-partial contracts plus unchanged prior gates; exact standard-Git application/postimage and behavioral RED remain pending B. A2 adds custom-draft, deferred-mount, abort/submit/cancel races and rejection cleanup; B adds hosted composition/type/read-only/execution gates and observes genuine RED. Production `007`/view `008` remain forbidden until B closes.
