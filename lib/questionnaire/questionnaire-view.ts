...
export class QuestionnaireView extends Container implements Focusable {
	...
	set focused(value: boolean) {
		this._focused = value;
		if (value && this.editingQuestion !== undefined) this.followCursor = true;
	...
	override handleMouse(event: TuiMouseEvent): ReturnType<Container["handleMouse"]> {
		...
		if (this.renderedRows !== this.tui.terminal.rows || event.width !== this.renderedWidth ||
			event.height !== this.lineOwners.length ||
	...
	override render(width: number): string[] {
		...
		if (this.renderedWidth !== viewport || this.renderedRows !== this.tui.terminal.rows) this.followCursor = true;
		...
		if (this.followCursor) {
			if (this.editingQuestion !== undefined) {
				const first = body.owners.findIndex((owner) => owner && "editorRow" in owner);
				if (first >= 0) {
					const height = (body.owners[first] as { editorHeight: number }).editorHeight;
					// Keep the native Editor (including its borders) together where it fits.
					if (first < this.bodyOffset) this.bodyOffset = first;
					if (first + height > this.bodyOffset + bodyHeight)
						this.bodyOffset = Math.max(0, first + height - bodyHeight);
					// The Editor owns its internal text scroll; follow its visible IME caret.
					const caret = body.lines.findIndex((line) => line.includes(CURSOR_MARKER));
					if (caret >= 0) this.bodyOffset = Math.max(caret - bodyHeight + 1,
						Math.min(this.bodyOffset, caret));
				}
			}
			else {
				...
				if (row >= 0) this.bodyOffset = Math.max(row - bodyHeight + 1, Math.min(this.bodyOffset, row));
			}
			this.bodyOffset = Math.max(0, Math.min(this.bodyOffset, this.bodyMaxOffset));
