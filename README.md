...
`collapseKey` is optional and defaults to `ctrl+]`. It accepts a trimmed, case-insensitive Pi key identifier such as `ctrl+k`; use `"off"` to disable collapse. The configured key replaces the questionnaire with the localized `{key} to expand · Esc to cancel` hint and restores any retained draft when pressed again. While collapsed, a scoped questionnaire input listener reopens the overlay; hosts without that listener retain the focused one-line fallback. This does not install a global shortcut.
